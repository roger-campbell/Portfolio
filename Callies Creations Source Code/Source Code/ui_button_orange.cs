...
function ui_button_orange_fruit::onAdd( % this) { % this.setUseMouseEvents(true);
 //$number_of_waiters = 0;
...
function ui_button_orange_fruit::onMouseMove( % this, % modifier, % worldPosition, % mouseClicks) {
 if ($THE_GAME_IS_PAUSED != 1) {
  if ($disable_all_remaining_fruit_ui != 1 & $unlock_subtle_red_mouseover == 1) {
   button_orange_fruit.BlendColor = "1 0.7 0.7 0.9";
  }
 }
}

function ui_button_orange_fruit::onMouseLeave( % this, % modifier, % worldPosition, % mouseClicks) {
 if ($THE_GAME_IS_PAUSED != 1) {
  ...
  if ($disable_all_remaining_fruit_ui != 1 & $tutorial_cue_tab_control != 1) {
   unred_out_everything();
  }
 }
}

function ui_button_orange_fruit::onMouseDown( % this, % modifier, % worldPosition, % mouseClicks) {
 if ($MASTER_ALL_BUTTONS_DISABLED != 1 & $disable_fruits_temporarily_due_to_swap_mousedown_error != 1) {
  if ($THE_GAME_IS_PAUSED != 1) {
   ...
   if ($tut_char_fruit_choices == 1 & $lock_down_isolater_mask_part_i_to_ii == 1 & $completely_unlock_first_fruit_iterator == 1 & $lock_down_first_fruit_iterator != 1) {
    $tut_iterate_orange += 1;
   }
   //edit 01-22-2014 make it so you can't perpetually click valid fruits during tut seg 2 just incase
   if ($tut_iterate_orange >= 2) {
    $tut_iterate_orange = 2;
   }
   ...
   if ($until_tut_char_is_activated_dont_do_fruits != 1 & % check_to_see_if_drink_out_blocked != 4) {
    ...
    if ($tut_char_fruit_choices != 2 & $toggle_off_orange_fruit_for_tut != 1) {
     ...
     if ($tut_yes_is_active == 1 & $lock_down_isolater_mask_part_i_to_ii == 1) {
      monster_shake.setScenePause(false);
      $toggle_off_orange_fruit_for_tut = 1;
     }
     ...
     button_orange_fruit.BlendColor = "0.5 0.5 0.5 1";
     //if inventory has enough, go ahead

     if ($fruit_frame == 0 & $inv_orange > 0) { 
	 % authenticate_this = 1;
      $inv_orange += 1; 
	  % inform_waiter_fruit_wanted_is = 2;
      echo("inv_orange: " @ $inv_orange);
      $identify_corresponding_image = "button_orange_fruit3ImageMap";
      $identify_corresponding_frame = 0;
     }

     if ($fruit_frame == 1 & $inv_peach > 0) { 
	 % authenticate_this = 1;
      $inv_peach += 1; 
	  % inform_waiter_fruit_wanted_is = 8;
      echo("inv_peach: " @ $inv_peach);
      $identify_corresponding_image = "button_orange_fruit3ImageMap";
      $identify_corresponding_frame = 1;
     }

     if ($fruit_frame == 2 & $inv_mango > 0) { 
	 % authenticate_this = 1;
      $inv_mango += 1; 
	  % inform_waiter_fruit_wanted_is = 14;
      echo("inv_mango: " @ $inv_mango);
      $identify_corresponding_image = "button_orange_fruit3ImageMap";
     ...
     if ($fruit_frame == 0 & $inv_orange <= 0) { % authenticate_this = 0;
     }

     if ($fruit_frame == 1 & $inv_peach <= 0) { % authenticate_this = 0;
     }

     if ($fruit_frame == 2 & $inv_mango <= 0) { % authenticate_this = 0;
     }
     $check_cue_number += 1;
     ...
     if ($decide_drink_part1 == 1 & % authenticate_this == 1 & $tut_iterate_orange != 2) {
...
function ui_button_orange_fruit::onMouseUp( % this, % modifier, % worldPosition, % mouseClicks) {
...
function ui_button_orange_fruit::set_cue_down() {
