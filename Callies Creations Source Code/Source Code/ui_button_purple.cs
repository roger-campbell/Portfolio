...
function ui_button_purple_fruit::onAdd( % this) { % this.setUseMouseEvents(true);
 //$number_of_waiters = 0;
...
function ui_button_purple_fruit::onMouseMove( % this, % modifier, % worldPosition, % mouseClicks) {
 if ($THE_GAME_IS_PAUSED != 1) {
  if ($disable_ui_and_rgbp_swap != 1 & $tut_char_fruit_choices != 1) {
   button_purple_fruit.BlendColor = "1 0.7 0.7 0.9";
  }
 }
}

function ui_button_purple_fruit::onMouseLeave( % this, % modifier, % worldPosition, % mouseClicks) {
 if ($THE_GAME_IS_PAUSED != 1) {
  if ($disable_ui_and_rgbp_swap != 1 & $tut_char_fruit_choices != 1) {
...
function ui_button_purple_fruit::onMouseDown( % this, % modifier, % worldPosition, % mouseClicks) {
  ...
  if ($THE_GAME_IS_PAUSED != 1) {
   ...
   if ($until_tut_char_is_activated_dont_do_fruits != 1 & % check_to_see_if_drink_out_blocked != 4) {
    ...
    if ($disable_ui_and_rgbp_swap != 1 & $tut_char_fruit_choices != 2) {
     ...
     button_purple_fruit.BlendColor = "0.5 0.5 0.5 1";
     //if inventory has enough, go ahead

     if ($fruit_frame == 0 & $inv_grapes > 0) { 
	 % authenticate_this = 1;
      $inv_grapes += 1; % inform_waiter_fruit_wanted_is = 6;
      echo("inv_grapes: " @ $inv_grapes);
      $identify_corresponding_image = "button_purple_fruit3ImageMap";
      $identify_corresponding_frame = 0;
     }

     if ($fruit_frame == 1 & $inv_blackberry > 0) { 
	 % authenticate_this = 1;
      $inv_blackberry += 1; 
	  % inform_waiter_fruit_wanted_is = 12;
      echo("inv_blackberry: " @ $inv_blackberry);
      $identify_corresponding_image = "button_purple_fruit3ImageMap";
      $identify_corresponding_frame = 1;
     }

     if ($fruit_frame == 2 & $inv_fig > 0) { 
	 % authenticate_this = 1;
      $inv_fig += 1; % inform_waiter_fruit_wanted_is = 18;
      echo("inv_fig: " @ $inv_fig);
      $identify_corresponding_image = "button_purple_fruit3ImageMap";
     ...
     if ($fruit_frame == 0 & $inv_grapes <= 0) { % authenticate_this = 0;
     }

     if ($fruit_frame == 1 & $inv_blackberry <= 0) { % authenticate_this = 0;
     }

     if ($fruit_frame == 2 & $inv_fig <= 0) { % authenticate_this = 0;
     }
     $check_cue_number += 1;
...
function ui_button_purple_fruit::onMouseUp( % this, % modifier, % worldPosition, % mouseClicks) {
...
function ui_button_purple_fruit::set_cue_down() {
